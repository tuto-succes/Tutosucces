...
export function TutorRegistrationPage({ onBack, onNavigateToContact }: TutorRegistrationPageProps) {
  ...
  const addTimeSlot = (jour: keyof WeeklyAvailability) => {
    setForm({
      ...
      disponibilites: {
        ...form.disponibilites,
        [jour]: [...form.disponibilites[jour], { debut: '09:00', fin: '17:00' }]
      }
    });
  };

  const removeTimeSlot = (jour: keyof WeeklyAvailability, index: number) => {
    const newSlots = form.disponibilites[jour].filter((_, i) => i !== index);
    setForm({
      ...form,
      disponibilites: {
        ...form.disponibilites,
                  ...
                  {joursOptions.map((jour) => (
                    <div key={jour.key} className="p-4 rounded-xl" style={{ backgroundColor: '#F8F9FA' }}>
                      <div className="flex items-center justify-between mb-3">
                        <h3 className="text-base font-bold" style={{ color: '#2C3E50' }}>
                          {jour.label}
                        </h3>
                        <button
                          type="button"
                          onClick={() => addTimeSlot(jour.key)}
                          className="px-4 py-2 rounded-lg text-sm font-semibold hover:scale-105 transition-transform"
                          style={{ backgroundColor: '#2E5CA8', color: '#FFFFFF' }}
                        >
                            ...
                            <div key={index} className="flex items-center gap-3 bg-white p-3 rounded-lg">
                              <div className="flex items-center gap-2 flex-1">
                                ...
                                <input
                                  type="time"
                                  value={slot.debut}
                                  onChange={(e) => updateTimeSlot(jour.key, index, 'debut', e.target.value)}
                                  className="px-3 py-2 rounded-lg border-2 border-gray-300 focus:outline-none focus:border-blue-500"
                                  style={{ color: '#2C3E50' }}
                                />
                                <label className="text-sm font-medium" style={{ color: '#2C3E50' }}>
                                  à
                                ...
                                <input
                                  type="time"
                                  value={slot.fin}
                                  onChange={(e) => updateTimeSlot(jour.key, index, 'fin', e.target.value)}
                                  className="px-3 py-2 rounded-lg border-2 border-gray-300 focus:outline-none focus:border-blue-500"
                                  style={{ color: '#2C3E50' }}
                                />
                              </div>
                              <button
                                type="button"
                                onClick={() => removeTimeSlot(jour.key, index)}
